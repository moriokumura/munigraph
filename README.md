...
- 自治体バージョンレコード (`municipality_versions.csv`) (4k〜)
  - 特定の期間における自治体の属性（名称、読み、支庁、郡）を保持
...
- 廃置分合イベントレコード (`change_events.csv`) (2k〜)
  ...
  - 同一自治体内の郡の変更は、バージョン間の差分としてアプリケーションが自動検知するため、このファイルには記述しない（支庁の変更は表示対象外）
...
- dateが同日に複数の変化（例：名称変更と市制施行）がある場合、アプリケーションがバージョン間の期間や属性（名称の末尾など）に基づいて論理的な順序を解決する。CSV側で順序を厳密に管理する必要はない。
...
- 各バージョンにおける属性（郡、現存自治体のみ支庁）や、その前後に発生した廃置分合イベントを表示
